...
function renderTrack(): void {
  ...
  const locationWidth =
    locationOptions.length > 0 ? calculateDropdownWidth(locationOptions) : 120;

  const distanceOptions = getAvailableDistances(track.trackName, track.surface);
  const distanceWidth =
    distanceOptions.length > 0 ? calculateDropdownWidth(distanceOptions) : 60;
  ...
  const fields: Field[] = [
    ...
    },
    {
      ...
      label: "Season",
      type: "select",
      options: ["<Random>", "Spring", "Summer", "Fall", "Winter", "Sakura"],
      width: calculateDropdownWidth([
        "<Random>",
        "Spring",
        "Summer",
        ...
        if (distanceSelect) {
          ...
          newDistanceOptions.forEach((dist) => {
            ...
            if (
              ...
              dist.toLowerCase() === currentDistanceStr?.toLowerCase()
            ) {
              option.selected = true;
            }
          ...
          const isCurrentDistanceValid = newDistanceOptions.some(
            (opt) =>
              ...
              opt.toLowerCase() === currentDistanceStr?.toLowerCase(),
          );
