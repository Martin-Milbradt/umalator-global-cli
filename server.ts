...
app.post("/api/config/:filename/duplicate", (req, res) => {
  try {
    ...
    if (!newName || typeof newName !== "string" || !newName.trim()) {
      res
        .status(400)
        .json({ error: "newName is required and must be a non-empty string" });
      return;
    }
    ...
    if (!existsSync(sourcePath)) {
      res
        .status(404)
        .json({ error: `Source config file "${filename}" not found` });
      return;
    }
    ...
    if (existsSync(targetPath)) {
      res
        .status(409)
        .json({ error: `Config file "${newName.trim()}" already exists` });
      return;
    }
