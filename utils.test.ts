...
describe("parseGroundCondition", () => {
    ...
    it("throws on invalid input", () => {
        expect(() => parseGroundCondition("invalid")).toThrow("Invalid ground condition: invalid");
    });
});
...
describe("parseStrategyName", () => {
    ...
    it("throws on invalid input", () => {
        expect(() => parseStrategyName("sprinter")).toThrow("Invalid strategy: sprinter");
    });
});
...
describe("createWeightedSeasonArray", () => {
    it("creates array with correct distribution", () => {
        ...
        const counts = result.reduce((acc, season) => {
...
describe("createWeightedWeatherArray", () => {
    it("creates array with correct distribution", () => {
        ...
        const counts = result.reduce((acc, weather) => {
...
describe("createWeightedConditionArray", () => {
    it("creates array with correct distribution", () => {
        ...
        const counts = result.reduce((acc, condition) => {
...
describe("findSkillIdByNameWithPreference", () => {
    ...
    it("prefers skill with cost > 0 when preferCostGreaterThanZero is true", () => {
        const result = findSkillIdByNameWithPreference("Dual Skill", skillNames, skillMeta, true);
        expect(result).toBe("skill002");
    });

    it("prefers skill with cost === 0 when preferCostGreaterThanZero is false", () => {
        const result = findSkillIdByNameWithPreference("Dual Skill", skillNames, skillMeta, false);
        expect(result).toBe("skill001");
    });

    it("returns null when no match found", () => {
        const result = findSkillIdByNameWithPreference("Unknown", skillNames, skillMeta, true);
        expect(result).toBeNull();
    });

    it("returns single match regardless of preference", () => {
        const result = findSkillIdByNameWithPreference("Unique Only", skillNames, skillMeta, true);
...
describe("findSkillVariantsByName", () => {
    ...
    it("finds ○ and ◎ variants when base has cost 0", () => {
        const result = findSkillVariantsByName("Right-Handed", skillNames, skillMeta);
    ...
    it("returns exact match when it has cost > 0", () => {
        ...
        const result = findSkillVariantsByName("Power Up", skillNamesWithCost, skillMetaWithCost);
...
describe("calculateStatsFromRawResults", () => {
    it("calculates statistics correctly for odd-length array", () => {
        ...
        const result = calculateStatsFromRawResults(rawResults, 100, 10, "Test Skill", 95);
    ...
    it("calculates median correctly for even-length array", () => {
        ...
        const result = calculateStatsFromRawResults(rawResults, 100, 0, "Test", 95);
...
describe("calculateSkillCost", () => {
    const skillMeta: Record<string, { baseCost: number; groupId?: number; order?: number }> = {
    ...
    it("calculates base cost without discount", () => {
        const result = calculateSkillCost("skill001", skillMeta, { discount: 0 });
        expect(result).toBe(200);
    });

    it("applies discount correctly", () => {
        const result = calculateSkillCost("skill001", skillMeta, { discount: 10 });
        expect(result).toBe(180);
    });

    it("rounds up after discount", () => {
        const result = calculateSkillCost("skill001", skillMeta, { discount: 15 });
        expect(result).toBe(170);
    });

    it("uses default cost of 200 for unknown skill", () => {
        const result = calculateSkillCost("unknown", skillMeta, { discount: 0 });
        expect(result).toBe(200);
    });

    it("handles null discount as 0", () => {
        const result = calculateSkillCost("skill001", skillMeta, { discount: null });
        expect(result).toBe(200);
    });
});
